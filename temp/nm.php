...
class NamuMark{
    ...
    public function doParse(){
        ...
        // offset 설정하기
        for($i=0;$i < $wt_strlen; $i++){
            $temp = ['pos' => $i];
	    $now = iconv_substr($this->wikitext,$i,1);
	    ...
	    if($line == '' && str_starts_with(iconv_substr($this->wikitext, $i), '|') && $temp = $this->tableParser($this->wikitext, $i, fn($v)=>$i = $v)){
                $tokens += $temp;
	        $line = '';
	        $now = '';
	        continue;
	    ...
	    if($line == '' && str_starts_with(iconv_substr($this->wikitext, $i), '>') && $temp = $this->blockquoteParser($this->wikitext, $i, fn($v) => $i = $v, fn($proc, $args) => $this->$proc($args[1], $args[2]))){
	        $tokens += $temp;
	        $line = '';
	        $now = '';
	        continue;
	    ...
	    foreach($multiBrackets as $bracket){
	        ...
	        if(str_starts_with(iconv_substr($this->wikitext, $i), $bracket['open']) && $temp = $this->bracketParser($this->wikitext, $i, fn($v) => $i = $v)){
	...
	return $tokens;
 	//var_dump($this->renderer->getResult());
    }
	...
	for($j=0;$j<$linecount;$j++){
		...
		foreach ($singleBrackets as $bracket){
		    ...
		    $innerStrLen = null;
                    ...
                    // 여는 브라켓으로 시작하고 파서가 작동한다면
		    if(str_starts_with(iconv_substr($line,$j),$bracket['open']) && ($temp = $this->bracketParser($line, $nj, $bracket, fn($v) => $nj = $v, fn($proc, $args) => $this->$proc($args[0], $args[1]), fn($v) => $innerStrLen = $v))){
			if(strlen($plainTemp) !== 0){
		            array_push($result, ['name' => 'plain', 'text' => $plainTemp]);
			    $plainTemp = '';
			}
	...
	for($i=$pos;$i<$txtstrlen;$i++){
		...
		$i += strlen($bracket['close']) - 1;
			...
			if($cnt == 0 && $done){
				...
				if($matchLenCallback) $matchLenCallback(iconv_strlen($innerString) + strlen($bracket['open']) + strlen($bracket['close']));
                                $setpos($i);
